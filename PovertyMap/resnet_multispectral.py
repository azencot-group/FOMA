...
class ResNet(nn.Module):
    ...
    def get_feats(self, x, y, layer=4, latent=False):
        ...
        if latent:
            x, y = get_batch_foma(self.args, x, y, latent=latent)
    ...
    def _forward_impl(self, x, y, latent=False, with_feats=False):
        x, y = self.get_feats(x, y, latent)
        feats = x
    ...
    def forward(self, x, with_feats=False):
        return self._forward_impl(x, None, with_feats)[0]

    def forward_foma(self, x, y, with_feats=False):
        return self._forward_impl(x, y, with_feats,latent=True)


class ResNet18(ResNet):
    def __init__(self, args, num_classes=10, num_channels=3):
        super().__init__(args,
